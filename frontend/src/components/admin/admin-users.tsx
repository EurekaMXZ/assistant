...
import { createAdminUser, listAdminUsers, resetAdminUserPassword, updateAdminUser } from "@/lib/api";
...
export function AdminUsers({ actor }: { actor: User }) {
  ...
  return (
    <div>
      <AdminPageHeader
        title="用户"
        action={<Button size="sm" onClick={() => openEditor("create")}><Plus />创建用户</Button>}
      />
                  ...
                  <tr key={item.id}>
                    <td className="py-3 pr-4">
                      <div className="flex items-center gap-3">
                        <span className="grid size-8 place-items-center rounded-md bg-muted"><UserRound className="size-4" /></span>
                        <div>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{formatAdminDate(item.last_login_at)}</td>
                      ...
                      {manageable ? (
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-40">
                            <DropdownMenuGroup>
                              <DropdownMenuItem onClick={() => openEditor(item)}>编辑资料</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => { setResetUser(item); setPassword(""); }}>
                                <KeyRound />重置密码
                              </DropdownMenuItem>
      ...
      <Dialog open={editor !== null} onOpenChange={(open) => !open && setEditor(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editor === "create" ? "创建用户" : "编辑用户"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            {actor.role === "system" ? (
              <div className="space-y-2">
                ...
                <select id="admin-user-role" className={adminSelectClass} value={role} onChange={(event) => setRole(event.target.value as Exclude<UserRole, "system">)}>
            ...
            {editor === "create" ? (
              <div className="space-y-2">
                ...
                <Input id="admin-user-password" type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" />
              </div>
            ) : null}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditor(null)}>取消</Button>
            <Button disabled={saving || !email.trim() || !username.trim() || (editor === "create" && password.length < 8)} onClick={() => void save()}>
      ...
      <Dialog open={!!resetUser} onOpenChange={(open) => !open && setResetUser(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>重置 {resetUser?.username} 的密码</DialogTitle></DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="reset-user-password">新密码</Label>
            <Input id="reset-user-password" type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" />
          </div>
          <DialogFooter>
