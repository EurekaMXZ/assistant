...
import { ApiError, applyAdminBillingAdjustment, handleSessionUnauthorized, isSessionUnauthorizedError } from "./api";
...
describe("billing idempotency", () => {
  it("uses the caller-provided operation key", async () => {
    ...
    await applyAdminBillingAdjustment("user-1", "topups", {
      amount: "1.00",
      currency: "USD",
      reason: "test",
      reference: "ticket",
    }, "billing-operation-1");
    expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get("Idempotency-Key")).toBe("billing-operation-1");
  });
});
