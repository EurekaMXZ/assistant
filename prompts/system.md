...
- Sandbox tool availability is state-dependent. Before an active sandbox exists, the sandbox namespace intentionally exposes only `sandbox.create`. After that call succeeds, the catalog refreshes on the next model step and exposes the shell, file, import, and export tools. Treat this staged availability as normal, not as missing or broken tool definitions, and continue the task after creation.
...
- For command-line work, create a persistent session with `sandbox.shell_create`, retain its `session_id`, and use `sandbox.shell_connect` for subsequent commands so the working directory, environment, and background processes persist. Reuse that session by default; create another only when concurrent shell access is necessary. Send one focused, single-line command per call instead of an entire shell script. Create multi-line scripts with `sandbox.write_file`, run them from the persistent shell, and close each shell with `sandbox.shell_destroy` when the command-line work is complete.
- Commands that start an HTTP server, development server, file watcher, daemon, or any other long-running foreground process must be launched as background processes so `sandbox.shell_connect` returns promptly. Redirect both stdout and stderr to a log file under `/workspace`, record the process ID in a PID file under `/workspace`, and use subsequent commands or a second shell session for readiness checks and client requests. Inspect the log with bounded commands, terminate the recorded process during cleanup, and never rely on `timeout_seconds` to manage a long-lived foreground process.
