...
func (l *ContextLoader) hydrateScheduledRunImages(ctx context.Context, state *ScheduledRunState) error {
	...
	for index := range state.Request.Input {
		hydrated, err := l.hydrateModelItemImages(ctx, state.Request.Input[index])
		if err != nil {
			return err
		}
...
func (l *ContextLoader) hydrateModelItemImages(ctx context.Context, item llm.ModelItem) (llm.ModelItem, error) {
	if len(item.Raw) == 0 {
		return item, nil
	}
	...
	if rawRef, ok := message["result_ref"]; ok {
		...
		if err := json.Unmarshal(rawRef, &ref); err != nil {
			return item, fmt.Errorf("decode generated image reference: %w", err)
		}
		result, err := l.hydrateImageReferenceBytes(ctx, ref)
		if err != nil {
			return item, err
		}
	...
	for index, rawPart := range content {
		...
		imageURL, err := l.hydrateImageReference(ctx, *part.ImageRef)
		if err != nil {
			return item, err
		}
		content[index], err = json.Marshal(map[string]string{"type": "input_image", "image_url": imageURL})
		if err != nil {
			return item, fmt.Errorf("marshal hydrated image: %w", err)
		}
		changed = true
	}
	if !changed {
	...
	if err != nil {
		return item, fmt.Errorf("marshal hydrated image content: %w", err)
	}
	message["content"] = encodedContent
	item.Raw, err = json.Marshal(message)
	if err != nil {
		return item, fmt.Errorf("marshal hydrated model item: %w", err)
	}
	return item, nil
}
